...
export const FunctionCommonSchema = z.object({ "call": z.string().describe("The name of the function to call."), "catalogId": z.string().describe("The catalog ID for this function, overriding any surface-level default catalogId.").optional() }).describe("Baseline envelope properties common to all function calls. Function-specific argument schemas ('args') are defined individually by each function in the active catalog.")
...
export const FunctionCallSchema = z.record(z.string(), z.any()).and(z.intersection(FunctionCommonSchema, z.any().superRefine((x, ctx) => {
  ...
  }))).describe("Invokes a named function, combining common function properties with the catalog function definition.")
