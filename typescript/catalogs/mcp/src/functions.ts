...
/**
 * The functions of the A2UI MCP catalog: `callMcpTool` and the data transformation functions
 * (`jmespath`, `split`, `regexCapture`, `regexReplace`, `updateDataModel`).
 *
 * This is the `@a2ui/catalog-mcp/functions` entry point. It imports nothing that needs a
 * browser, so an agent or a test running in Node can build the functions without a DOM; the
 * package's main entry point adds the `McpApp` component on top of it.
 *
 * Example setup:
 * ```ts
