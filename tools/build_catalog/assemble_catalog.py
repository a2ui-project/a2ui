...
BASIC_CATALOG_URLS = {
    ...
    "1.0": (
        "https://raw.githubusercontent.com/a2ui-project/a2ui/refs/heads/main/catalogs/v1_0/basic/catalog.json"
    ),
}
...
class CatalogAssembler:
        ...
        try:
            if is_remote_uri(uri):
                ...
                req = urllib.request.Request(
                    uri, headers={"User-Agent": "A2UI-Assembler/1.0"}
                )
            ...
            if category in source:
                for k, v in source[category].items():
                    if k in target[category]:
                        logger.warning(
                            f"{label} collision: '{k}' already exists. Overwriting."
                        )
                    target[category][k] = v
    ...
    def _synthesize_union_types(
        self, combined_catalog: dict, merged_theme: dict
    ) -> None:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Custom catalogId for the output. Defaults to urn:a2ui:catalog:<base_name>"
        ),
    )
    parser.add_argument(
        "--version",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Additional Markdown instructions file(s) to include in the combined"
            " catalog (can be specified multiple times)"
        ),
    )
    parser.add_argument(
        "--out-dir",
