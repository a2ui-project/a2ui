...
from ..common.events import EventSource, Subscription, Signal
...
class SurfaceModel:
    ...
    def __init__(
        ...
        # Track model changes
        self._comp_created_sub = self.components_model.on_created.subscribe(
            self._on_component_created
        )
        self._comp_deleted_sub = self.components_model.on_deleted.subscribe(
    ...
    def get_or_create_node(self, component_id: str, data_path: str) -> ComponentNode:
        ...
        component_model = self.components_model.get(component_id)
        props_signal = Signal({})
        ...
        data_context = DataContext(
            path=data_path, data_model=self.data_model, catalog=self.catalog
        )
        comp_context = ComponentContext(
            component_model=component_model,
            data_context=data_context,
            surface_components=self.components_model,
            dispatch_action_callback=self.dispatch_action,
        )
            ...
            for k, v in list(new_props.items()):
                if k == "action" or (
                    ...
                    def make_action_closure(payload=action_payload):
                        ...
                        self.dispatch_action(resolved_payload, component_id)
            ...
            for list_ref in list_refs:
                if list_ref in new_props:
                    ...
                    if isinstance(val, list):
                        ...
                        sub = self.data_model.subscribe(template_path, on_array_changed)
    ...
    def dispose(self) -> None:
        """Disposes of the surface and its resources, cleaning up all active nodes."""
        if hasattr(self, "_comp_created_sub"):
            self._comp_created_sub.unsubscribe()
        if hasattr(self, "_comp_deleted_sub"):
