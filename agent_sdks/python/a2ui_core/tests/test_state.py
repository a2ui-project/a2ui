...
def test_surface_model_node_layer_resolution():
    ...
    node = surface.get_or_create_node("root", "/")
    assert node.type == "Placeholder"
    assert "root" in surface.active_nodes
    ...
    assert surface.rootNode.value is not None
    assert surface.rootNode.value.type == "Column"
    ...
    child_node = surface.get_or_create_node("child_1", "/")
    ...
    assert surface.get_or_create_node("child_1", "/").type == "Placeholder"

    surface.dispose()


def test_signal_reactive_behavior():
    sig = Signal(10)
    assert sig.value == 10
    ...
    sig.value = 20  # No change, shouldn't emit
    assert emitted == [10, 20]
...
def test_surface_model_root_node_resolution():
    ...
    assert surface.rootNode.value is None
    ...
    assert surface.rootNode.value is not None
    assert isinstance(surface.rootNode.value, ComponentNode)
    assert surface.rootNode.value.component_id == "root"
    assert surface.rootNode.value.type == "Column"
...
def test_node_layer_reactive_property_resolution():
    ...
    root_node = surface.rootNode.value
...
def test_data_context_function_call_bindings():
    ...
    node_1 = surface.get_or_create_node("text-1", "/")
    ...
    node_2 = surface.get_or_create_node("text-2", "/")
    ...
    node_3 = math_surface.get_or_create_node("text-3", "/")
...
def test_a2ui_text_renderer_reactive_layouts():
    ...
    renderer = A2uiTextRenderer(surface)
