...
def test_parses_literal_strings_unchanged(parser: ExpressionParser) -> None:
    assert parser.parse("hello world") == ["hello world"]


def test_parses_simple_interpolation(parser: ExpressionParser) -> None:
    assert parser.parse("hello ${foo}") == ["hello ", {"path": "foo"}]


def test_parses_number_interpolation(parser: ExpressionParser) -> None:
    assert parser.parse("number is ${num}") == ["number is ", {"path": "num"}]


def test_parses_nested_interpolation(parser: ExpressionParser) -> None:
    assert parser.parse("val is ${${nested}}") == ["val is ", {"path": "nested"}]


def test_handles_escaped_interpolation(parser: ExpressionParser) -> None:
    assert parser.parse("escaped \\${foo}") == ["escaped ", "${", "foo}"]


def test_parses_function_calls(parser: ExpressionParser) -> None:
...
def test_parses_function_calls_with_string_literals(parser: ExpressionParser) -> None:
...
def test_parses_keywords(parser: ExpressionParser) -> None:
    assert parser.parse("${true} ${false} ${null}") == [True, " ", False, " "]


def test_returns_error_on_max_depth_exceeded(parser: ExpressionParser) -> None:
    with pytest.raises(ValueError, match="Max recursion depth reached"):
        parser.parse("depth", 11)


def test_handles_deep_recursion_gracefully(parser: ExpressionParser) -> None:
    assert parser.parse('${${"hello"}}') == ["hello"]


def test_returns_error_on_unclosed_interpolation(parser: ExpressionParser) -> None:
    with pytest.raises(ValueError, match="Unclosed interpolation"):
        parser.parse("hello ${world")


def test_returns_error_on_invalid_function_syntax(parser: ExpressionParser) -> None:
    with pytest.raises(ValueError, match="Expected '\\)'"):
        parser.parse("${add(a: 1, b: 2}")


def test_returns_error_on_unexpected_characters_at_end(
...
def test_handles_empty_identifiers(parser: ExpressionParser) -> None:
...
def test_handles_parsing_paths_with_special_characters(
    parser: ExpressionParser,
) -> None:
...
def test_returns_error_on_missing_colon_in_function_args(
    parser: ExpressionParser,
) -> None:
    with pytest.raises(ValueError, match="Expected ':'"):
