...
class PackSpecsBuildHook(BuildHookInterface):
  ...
  def initialize(self, version, build_data):
    ...
    express_dir = os.path.join(project_root, "src", "a2ui", "experimental", "express")
    ...
    if os.path.exists(g4_path):
      ...
      if not os.path.exists(cmd_path):
        raise RuntimeError(
            ...
            f" {cmd_path}. Please make sure the project dependencies (specifically"
            " antlr4-tools) are installed."
        )
