...
class ElementalCompiler:
  ...
  def compile(
    ...
    for child in root.children:
      ...
      elif child.tag == "script" and child.attrs.get("type") == "application/json":
        if "slot" not in child.attrs:
          try:
            data_model = json.loads(child.text.strip())
          except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON in root dataModel script: {e}") from e
        else:
        ...
        if (
          ...
          if slot_name:
            try:
              ...
              raise ValueError(
                  f"Invalid JSON in script slot '{slot_name}' of component"
                  f" '{comp_id}': {e}"
              ) from e
    else:
