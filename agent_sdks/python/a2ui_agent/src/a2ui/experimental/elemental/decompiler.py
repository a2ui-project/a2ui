...
class ElementalDecompiler:
    ...
    for c in components:
      ...
      for prop_name in props_to_check:
        if prop_name in c:
          ...
          if prop_name == "template" or _is_component_reference_property(p_schema):
            if isinstance(val, list):
              for v in val:
                if isinstance(v, str):
  ...
  def _render_component(self, comp_id: str, indent: int = 0, slot: Optional[str] = None) -> str:
    ...
    for prop_name in all_props:
      ...
      p_schema = self.helper.get_property_schema(comp_name, prop_name)
      is_ref = _is_component_reference_property(p_schema) or prop_name == "template"
      ...
      if is_ref:
        if prop_name == "template":
          ...
          if isinstance(val, list):
            for t_id in val:
              template_children.append(self._render_component(t_id, indent + 2))
          elif isinstance(val, str):
            template_children.append(self._render_component(val, indent + 2))
          ...
          if isinstance(val, list):
            for c_id in val:
              child_elements.append(self._render_component(c_id, indent + 1))
          elif isinstance(val, str):
            child_elements.append(self._render_component(val, indent + 1))
          elif isinstance(val, dict) and "path" in val and "componentId" in val:
            # Render template binding as parent 'path' attribute and nested <template>
            path_val = {"path": val["path"]}
            attrs.append(self._format_attribute("path", path_val, self.comp_ids))
            template_child_html = self._render_component(val["componentId"], indent + 2)
            child_elements.append(
                f'{"  " * (indent + 1)}<template>\n{template_child_html}\n{"  " * (indent + 1)}</template>'
            )
        else:
  ...
  def _decompile_value_internal(self, val: Any, comp_ids: set[str]) -> str:
    ...
    if isinstance(val, list):
      list_reprs = [self._decompile_value_internal(item, comp_ids) for item in val]
