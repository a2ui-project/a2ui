...
/// disposed.
///
/// Reference classification is shared with graph validation: wire `$ref`
/// pointers, Dart `REF:` descriptions, local aliases, and structural
/// `ChildList` shapes identify child properties. An unmarked string property
/// is not a child reference.
///
...
class NodeResolver<T extends ComponentApi> {
  ...
  MutableComponentNode<T> _createNode(
    ...
    final void Function() unsubscribe = binder.resolvedProps.subscribe((_) {
  ...
  void _disposeNode(MutableComponentNode<T> node) {
    ...
    _pendingParents.removeWhere((componentId, waiting) {
      waiting.remove(node);
