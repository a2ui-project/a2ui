...
class AccessibilityAttributes(StrictBaseModel):
    ...
    label: DynamicString | None = Field(None, description="A short string, typically 1 to 3 words, used by assistive technologies to convey the purpose or intent of an element. For example, an input field might have an accessible label of 'User ID' or a button might be labeled 'Submit'.")
    description: DynamicString | None = Field(None, description="Additional information provided by assistive technologies about an element such as instructions, format requirements, or result of an action. For example, a mute button might have a label of 'Mute' and a description of 'Silences notifications about this conversation'.")
    live: Literal["off", "polite", "assertive"] | None = Field(description="Controls screen reader announcements for dynamic updates (WAI-ARIA aria-live). 'polite' waits for user pause; 'assertive' interrupts immediately for alerts.", default="off")
    hidden: DynamicBoolean | None = Field(None, description="Hides the element and its children from assistive technologies when true. Default is false.")
...
class ComponentCommon(StrictBaseModel):
    ...
    catalog_id: str | None = Field(None, alias="catalogId", description="The catalog ID for this component, overriding any surface-level default catalogId.")
    accessibility: AccessibilityAttributes | None = Field(None)
    metadata: dict[str, Any] | None = Field(None, description="Optional component-level metadata for vendor extensions.")
...
def _validate_literal_object(v: Any) -> dict[str, Any]:
    ...
    if found:
        raise ValueError(
            f"Object in DynamicValue cannot contain forbidden properties: {', '.join(sorted(found))}"
        )
    return v
...
DynamicValue = str | float | bool | list[Any] | DataBinding | FunctionCall | LiteralObject


class FunctionCommon(StrictBaseModel):
    model_config = ConfigDict(populate_by_name=True)
    catalog_id: str | None = Field(None, alias="catalogId", description="The catalog ID for this function, overriding any surface-level default catalogId.")


class IndexSystemFunctionArgs(StrictBaseModel):
    model_config = ConfigDict(populate_by_name=True)
    offset: DynamicNumber | None = Field(description="Optional. An offset to add to the 0-based index (e.g., 1 for 1-based indexing). Defaults to 0.", default=0)
...
class CheckRule(StrictBaseModel):
    ...
    model_config = ConfigDict(populate_by_name=True)
    condition: DataBinding | FunctionCall = Field(..., description="Path or function call evaluating to a structured validation result object.")
...
class ActionEvent(StrictBaseModel):
    ...
    model_config = ConfigDict(populate_by_name=True)
    name: str = Field(..., description="The name of the action to be dispatched to the agent.")
    user_message: DynamicString | None = Field(None, alias="userMessage", description="An optional human-readable message describing the action performed by the user, to present in conversation history or user feedback.")
    context: dict[str, DynamicValue] | None = Field(None, description="A JSON object containing the key-value pairs for the action context. Values can be literals or paths. Use literal values unless the value must be dynamically bound to the data model. Do NOT use paths for static IDs.")
...
class FunctionResponse(StrictBaseModel):
    ...
    model_config = ConfigDict(populate_by_name=True)
    function_call_id: str = Field(..., alias="functionCallId", description="The unique ID matching the initiating function call.")
    value: Any | None = Field(None, description="The return value of the function.")
    error: FunctionResponseError | None = Field(None, description="An error object indicating failure of the function execution.")
