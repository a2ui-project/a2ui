...
class CreateSurface(StrictBaseModel):
    ...
    theme: Any | None = Field(None, description="Theme parameters for the surface (e.g., {'primaryColor': '#FF0000'}). These must validate against the 'theme' schema defined in the catalog.")
    send_data_model: bool | None = Field(None, alias="sendDataModel", description="If true, the client will send the full data model of this surface in the metadata of every A2A message sent to the server that created the surface. Defaults to false.")
...
class UpdateComponents(StrictBaseModel):
    ...
    model_config = ConfigDict(populate_by_name=True)
    surface_id: str = Field(..., alias="surfaceId", description="The unique identifier for the UI surface to be updated.")
...
class UpdateDataModel(StrictBaseModel):
    ...
    value: Any | None = Field(None, description="The data to be updated in the data model. If present, the value at 'path' is replaced (or created). If omitted, the key at 'path' is removed.")
...
ServerToClientMessage = CreateSurfaceMessage | UpdateComponentsMessage | UpdateDataModelMessage | DeleteSurfaceMessage
