...
Envelope parsing takes no catalog: the protocol version tag and the single-update-type rule read none. Make it static on `A2uiValidator`, so a payload can be parsed before each message is matched to a surface, and so to a catalog.
...
| :----------------------- | :------------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------ | :-------------------- |
| **Protocol Envelope**    | Single update type per message (`createSurface`, `updateComponents`, etc.)                        | `A2uiValidator.parseMessages()` (static, no catalog needed)               | `A2uiValidationError` |
| **Protocol Envelope**    | Valid `version` tag (`v0.8`, `v0.9`, `v1.0`) & required envelope keys                             | `A2uiValidator.parseMessages()` (static, no catalog needed)               | `A2uiValidationError` |
...
| **Component Properties** | Property schema validation against catalog definition                                             | `A2uiValidator.validateComponent()`                                       | `A2uiValidationError` |
| **Function Arguments**   | Function call arguments validated against the catalog's function schema                           | `A2uiValidator.validateFunction()`                                        | `A2uiValidationError` |
| **Theme / Properties**   | `Theme` / `surfaceProperties` validation against catalog schema                                   | `A2uiValidator.validateTheme()`                                           | `A2uiValidationError` |
