...
> [!IMPORTANT]
> In order to run the `gh` command line tool, you have to run it with `env -u GITHUB_TOKEN`
> to unset the dummy environment variable (e.g., `env -u GITHUB_TOKEN gh auth status`).
...
> [!IMPORTANT]
> In order to run the `gh` command line tool, you have to run it with `env -u GITHUB_TOKEN`
> to unset the dummy environment variable (e.g., `env -u GITHUB_TOKEN gh pr create`).
