...
# Must match FLAG_LABEL in scripts/triage.mjs, which marks the triage queue.
FLAG_LABEL = "status: needs-triage"

# Parks an item the team is actively discussing. The second-line triage queue in
# docs/contributing/triage.md excludes issues carrying it, so it takes the issue
# out of the queue without pretending a decision has been made.
IN_DISCUSSION_LABEL = "status: in-discussion"
...
def apply_decision(dec):
    """Applies one triage decision to GitHub.

    Shared by the batch CLI below and by the dashboard's per-issue Apply button
    (see launch_dashboard.py), so both paths write labels, assignees, comments
    and closures the same way. Raises on failure; the callers decide whether to
    report and continue or to surface the error to the browser.
    """
    issue_id = str(dec["id"])
    ...
    if issue_info_json:
        try:
            ...
            current_labels = [
                l.get("name") for l in issue_info.get("labels", []) if l.get("name")
            ]
    ...
    # Drop the queue flag now instead of waiting for the next automation run. The
    # label stays owned by scripts/triage.mjs: if the issue still matches a rule
    # after this decision, the next run puts it back within a minute.
    if FLAG_LABEL in current_labels:
        remove_labels.append(FLAG_LABEL)

    # Apply label changes via gh CLI
    if remove_labels:
        remove_str = ",".join(remove_labels)
        print(f"  Removing labels: {remove_str}")
...
def mark_in_discussion(issue_id):
    """Parks an issue as under team discussion instead of triaging it.

    Used by the dashboard as the alternative to applying a decision: nothing
    else about the issue changes, so the triage decision stays open until the
    team has settled it. The queue flag is left in place — scripts/triage.mjs
    owns it, and the oncall query filters on IN_DISCUSSION_LABEL instead.
    """
    issue_id = str(issue_id)
    print(f"  Adding label: {IN_DISCUSSION_LABEL}")
    run_cmd(["gh", "issue", "edit", issue_id, "--add-label", IN_DISCUSSION_LABEL])
