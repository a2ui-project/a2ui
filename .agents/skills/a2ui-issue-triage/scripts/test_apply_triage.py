...
class TestApplyTriage(unittest.TestCase):
    ...
    @patch("apply_triage.run_cmd")
    @patch("os.path.exists")
    def test_needs_info_applies_waiting_label(
        self, mock_exists, mock_run, mock_parse_args
    ):
        # The 'needs_info' action must apply the waiting label even when the
        # decision's label list omits it, since the oncall engineer can switch the
        # action in the dashboard after the suggestion was generated.
        mock_exists.return_value = True
        mock_args = MagicMock()
        mock_args.decisions_file = "/mock/triage_decisions.json"
        mock_parse_args.return_value = mock_args
        ...
        mock_decisions_data = {
            "decisions": [{
                ...
                "action": "needs_info",
                "labels": ["type: bug"],
                "reply": "Please provide reproduction steps.",
            }]
        }

        def run_cmd_side_effect(args):
            cmd_str = " ".join(args)
            if "view 120 --json labels" in cmd_str:
                return json.dumps({"labels": []})
            elif "view 120 --json comments" in cmd_str:
                return json.dumps({"comments": []})
            return ""
        ...
        mock_run.assert_any_call([
            ...
            "120",
            "--add-label",
            "P2,type: bug,status: waiting-for-user-response",
        ])
        # The issue stays open: 'needs_info' is not a closing action.
        for made_call in mock_run.call_args_list:
            self.assertNotIn("close", made_call.args[0])

    @patch("argparse.ArgumentParser.parse_args")
    @patch("apply_triage.run_cmd")
    @patch("os.path.exists")
    def test_needs_info_does_not_duplicate_waiting_label(
        self, mock_exists, mock_run, mock_parse_args
    ):
        # An issue that already carries the waiting label must not have it re-added.
        mock_exists.return_value = True
        mock_args = MagicMock()
        mock_args.decisions_file = "/mock/triage_decisions.json"
        mock_parse_args.return_value = mock_args
        ...
        mock_decisions_data = {
            "decisions": [{
                ...
                "assignee": "",
                "action": "needs_info",
                "labels": ["status: waiting-for-user-response"],
                "reply": "",
            }]
        }
        ...
        def run_cmd_side_effect(args):
            cmd_str = " ".join(args)
            if "view 121 --json labels" in cmd_str:
                return json.dumps(
        ...
        self.assertEqual(cm.exception.code, 0)
        for made_call in mock_run.call_args_list:
            self.assertNotIn("--add-label", made_call.args[0])
