...
1.  **Verify Eligibility**:
    - **For Required Features**: Find all codebase blueprints (`codebase.blueprint.md`) in the repository that implement any of the affected modules (where `associated_module` matches the module). Ensure that all these codebases have successfully implemented the feature (listed in their `codebase.blueprint.md` under `implemented_features`).
