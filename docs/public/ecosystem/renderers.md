...
| Renderer                                                           | Platform                              | v0.8 | v0.9 | v1.0 | Activity                                                                                                                                                                                                                                     | Links                                                                                                                                                                                           |
| ------------------------------------------------------------------ | ------------------------------------- | ---- | ---- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
**ChaliceForAuri/a2ui-svelte** (`svelte-a2ui`) is a Svelte 5 (runes) renderer for A2UI **v1.0**. It covers the full Basic Catalog — all 18 components, including the icon set and the `{svgPath}` variant — plus scope-aware data binding with collection templates, `checks`-driven validation, action round-trips with hand-picked context, renderer capability metadata, and adapter-style transports for HTTP/JSONL, AG-UI, and A2A. The protocol layer is pure (no Svelte imports — `reduce(state, message)` is a plain function) with structural sharing so unchanged components skip re-render, and the catalog registry is the security boundary: `on*` props are stripped unconditionally and nothing agent-supplied ever becomes a function. Wire behavior is verified against this repo's own fixtures, including a full replay of the contact-form example. The same author ships [auri](https://chaliceforauri.github.io/auri/) — agent-facing component catalogs (ops dashboards, approvals, charts) built on the renderer as contract + prompt-pack + implementation.
