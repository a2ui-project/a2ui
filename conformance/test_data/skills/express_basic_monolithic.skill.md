...
15. Surface targeting: Do NOT output `createSurface` or `surface(...)` commands. The active surface context is automatically managed. Simply output component declarations starting with `root = Component(...)` and data model assignments starting with `$/path = ...`.
