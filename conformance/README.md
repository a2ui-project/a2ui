...
- `agent/inference_format.yaml`: Contains test cases for prompt rendering by the inference formats (generate_prompt), plus the frozen `select_catalog` cases the shipped Python and Kotlin agent SDKs still run against.
...
- `agent/catalog_transformer.yaml`: Contains test cases for narrowing a catalog before prompting, including the `$defs/anyComponent` union that references the pruned entries.
