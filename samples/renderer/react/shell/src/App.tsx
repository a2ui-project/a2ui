...
export function App() {
  ...
  const sendAndProcessRef = useRef<
    ((message: A2uiRendererMessage | string) => Promise<void>) | null
  >(null);
...
function ShellContent({config, client, sendAndProcessRef, processor}: ShellContentProps) {
  ...
  const getMockResponse = useCallback((message: A2uiRendererMessage | string): A2uiMessage[] => {
    // Handle user actions
  ...
  const sendAndProcess = useCallback(
    async (message: A2uiRendererMessage | string) => {
      try {
        setRequesting(true);
        setError(null);
