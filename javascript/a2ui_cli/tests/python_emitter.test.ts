...
describe('PythonEmitter in @a2ui/cli', () => {
  ...
  it('emits single-file python catalog module with dataclass components and types', () => {
    ...
    try {
      ...
      assert.ok(content.includes('from a2ui.builder.v0_9 import ('));
    } finally {
      fs.rmSync(tmpDir, {recursive: true, force: true});
    }
  });

  it('resolves baseImport dynamically based on specVersion', () => {
    const catalog = Catalog.fromSchema(basicCatalogJson);
    const analysedV09 = CatalogAnalyzer.analyze(catalog, {specVersion: 'v0.9.1'});
    const emitterV09 = new PythonEmitter(analysedV09);
    assert.strictEqual(emitterV09.baseImport, 'a2ui.builder.v0_9');

    const analysedV10 = CatalogAnalyzer.analyze(catalog, {specVersion: 'v1.0'});
    const emitterV10 = new PythonEmitter(analysedV10);
    assert.strictEqual(emitterV10.baseImport, 'a2ui.builder.v1_0');

    const customEmitter = new PythonEmitter(analysedV09, {baseImport: 'custom.pkg'});
    assert.strictEqual(customEmitter.baseImport, 'custom.pkg');
  });
