...
- **Surface Configuration Refactoring**: Renamed `theme` to `configuration` at both the message and catalog level. Removed the `primaryColor` property entirely to decouple structural UI layout and branding customization from surface definition.
...
- Renamed the `$defs/theme` schema to `$defs/configuration` in both the basic and minimal catalogs, and removed the `primaryColor` property from it.
...
- Renamed `theme` capability block to `configuration` within the Catalog definition in `client_capabilities.json`.
...
- Updated `createSurface` message to rename the `theme` field to `configuration` (pointing to catalog configuration definitions).
