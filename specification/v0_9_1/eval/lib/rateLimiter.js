...
class RateLimiter {
    ...
    reportError(modelConfig, error) {
        const isResourceExhausted = error?.status === 'RESOURCE_EXHAUSTED' ||
            error?.code === 429 ||
        ...
        if (isResourceExhausted) {
            ...
            logger_1.logger.verbose(`RateLimiter: Pausing ${modelConfig.name} for ${pauseDuration}ms due to 429 error. Resuming at ${new Date(pausedUntil).toISOString()}`);
        }
    }
    async acquirePermit(modelConfig, tokensCost = 0) {
        this._waitingCount++;
        try {
            ...
            while (true) {
                ...
                if (pausedUntil && pausedUntil > Date.now()) {
                    ...
                    logger_1.logger.verbose(`Rate limiting ${name}: Paused by circuit breaker for ${pauseWait}ms`);
                ...
                logger_1.logger.debug(`RateLimiter check for ${name}: Cost=${tokensCost}, CurrentTokens=${currentTokens}, Limit=${effectiveTokensPerMinute}, Requests=${currentRequests}, RPM=${requestsPerMinute}`);
                // Check RPM
                ...
                logger_1.logger.verbose(`Rate limiting ${name}: Waiting ${requiredWait}ms (RPM wait: ${rpmWait}ms, TPM wait: ${tpmWait}ms)`);
    ...
    recordUsage(modelConfig, tokensUsed, isRequest = true) {
