...
The A2UI evaluation framework located in [eval/](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval) tests whether Large Language Models generate valid and contextually appropriate A2UI JSON payloads. The current framework runs Inspect AI tasks ([eval/tasks.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/tasks.py)) against prompt datasets stored in YAML format. Datasets are encrypted at rest using Transcrypt to prevent data contamination from web crawlers.
...
2. **Disconnected single-string prompts**: The existing schema ([eval/datasets/dataset_schema.json](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/datasets/dataset_schema.json)) uses a standalone `promptText` string. Real agentic applications pass a structured `messages` list containing prior user queries, assistant responses, and tool calls with execution outputs before the model generates UI.
...
Dataset files will be organized under [eval/datasets/](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/datasets) as separate modular YAML files. The legacy monolithic files (`prompts_v0_9_1.yaml` and `prompts_v1_0.yaml`) are migrated directly into named dataset files:
...
The CLI runner in [eval/main.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/main.py) and the CI runner in [eval/bin/run_ci_evals.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/bin/run_ci_evals.py) will support filtering by dataset:
...
The evaluation reporting tools in [eval/bin/report_evals.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/bin/report_evals.py) will group and calculate metrics per dataset:
...
| **`protocol_role`** | string | No | **The A2UI persona instruction passed to the SDK prompt generator.** Specifies the high-level role for UI generation (Default: *"You are an AI assistant. Based on the following request, generate a stream of JSON messages that conform to the provided JSON Schemas."*). Most contributors can leave this omitted. |
...
* **`system_prompt`** represents the **domain application persona**. In a real application, an agent has domain rules unrelated to A2UI (for example: *"You are the Acme Airlines Agent. Always confirm passenger baggage before checkout."*).
...
* Standard protocol rules (e.g. *"Construct the UI on surfaceId 'main'"* and *"Ensure there is one root component with id 'root'"*) are injected by default.
...
The schema in [eval/datasets/dataset_schema.json](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/datasets/dataset_schema.json) validates the clean dataset structure with `catalog` required:
...
When executing an evaluation sample, the solver in [eval/a2ui_eval/strategies/format.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/a2ui_eval/strategies/format.py) coordinates prompt merging, chat message list assembly, inference, and scoring.
...
1. **SDK prompt generation**: The A2UI SDK `TransportPromptGenerator` loads the required `catalog` and generates protocol rules, JSON schemas, component catalog definitions, and output tag requirements (`<a2ui-json>`).
...
1. **Programmatic validation ([a2ui_scorer](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/a2ui_eval/scorers.py))**:
   * Resolves the required `catalog` path specified in the data point.
   * Parses JSON from `<a2ui-json>` tags.
   * Runs the catalog validator ([A2uiValidator](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/agent_sdks/python/a2ui_agent/README.md)) to verify schema adherence, parent-child references, and root component presence.
2. **LLM judge scoring ([measured_model_graded_qa](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/a2ui_eval/scorers.py))**:
...
*(Request the password from an A2UI maintainer if you do not have it.)*
...
### Automated migration script (`eval/bin/migrate_datasets.py`)

A dedicated migration script automates the transformation of existing datasets into the modern `messages` format with the required `catalog` field:
...
def migrate_sample(item: dict, dataset_name: str) -> dict:
    ...
    new_item = {
        ...
        "catalog": item.get("catalog") or "specification/{version}/catalogs/basic/catalog.json",
    }

    # Transform promptText to messages array
    if "messages" in item:
        new_item["messages"] = item["messages"]
    elif "promptText" in item:
        new_item["messages"] = [{"role": "user", "content": item["promptText"]}]
    else:
        raise ValueError(f"Sample {item.get('name')} has neither messages nor promptText.")
    ...
    if "protocol_role" in item or "role_description" in item:
        new_item["protocol_role"] = item.get("protocol_role") or item.get("role_description")
    if "generation_rules" in item or "workflow_description" in item:
        new_item["generation_rules"] = item.get("generation_rules") or item.get("workflow_description")
...
def main():
    ...
    for old_file, new_file in MIGRATIONS.items():
        ...
        print(f"Successfully migrated {old_file} -> {new_file} ({len(migrated_data)} samples).")
...
To prevent failures if a developer branch or scratch file still contains legacy fields during the transition period, the dataset loader in [eval/a2ui_eval/dataset.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/a2ui_eval/dataset.py) includes a runtime normalizer:
...
2. **Run migration script**: Execute the automated migration script:
   ```bash
   uv run python bin/migrate_datasets.py
   ```
...
| :--- | :--- | :--- |
| [eval/datasets/dataset_schema.json](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/datasets/dataset_schema.json) | Update | Require `catalog`, use `messages` for chat turns, and include `dataset`, `system_prompt`, `protocol_role`, and `generation_rules` fields. |
| [eval/datasets/core_v0_9_1.yaml](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/datasets) | Create | Migrated v0.9.1 data points using the `messages` format and explicit `catalog`. |
| [eval/datasets/core_v1_0.yaml](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/datasets) | Create | Migrated v1.0 data points using the `messages` format and explicit `catalog`. |
| [eval/a2ui_eval/dataset.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/a2ui_eval/dataset.py) | Update | Add dataset discovery, `messages` array conversion to Inspect AI `ChatMessage` objects, required `catalog` enforcement, and validation CLI. |
| [eval/a2ui_eval/strategies/format.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/a2ui_eval/strategies/format.py) | Update | Implement domain system prompt merging and `messages` chat history prepending. |
| [eval/a2ui_eval/scorers.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/a2ui_eval/scorers.py) | Update | Use explicit `catalog` path in `a2ui_scorer`. |
| [eval/tasks.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/tasks.py) | Update | Point to migrated datasets and accept `dataset` and `datasets` filtering parameters. |
| [eval/main.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/main.py) | Update | Add `--dataset` and `--datasets` CLI flags. |
| [eval/bin/report_evals.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/bin/report_evals.py) | Update | Add per-dataset grouping in console summaries and markdown reports. |
| [eval/bin/run_ci_evals.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/bin/run_ci_evals.py) | Update | Pass dataset filtering arguments to `main.py`. |
| [eval/tests/test_dataset.py](file:///Users/jsimionato/development/a2ui_repos/datapoint/A2UI/eval/tests/test_dataset.py) | Update | Add unit tests for dataset loading, `messages` chat turn parsing, required `catalog` check, and schema validation. |
