...
| **`.agents/skills/a2ui-release-sdks/scripts/check_status.py`**   | **Implemented** | Centralized release status & PR creator script. Supports status inspection, git commit audit since last baseline tag, strict SemVer calculation, version string / changelog header mutations, lockfile sync, HTTP 404 registry handling, and PR creation via `gh`. Supports `--create-pr` and `--dry-run`. |
...
| **`.github/workflows/release-pr.yml`**                           | **Implemented** | Created GitHub Action workflow for weekly scheduled cron (`0 9 * * 1`) and `workflow_dispatch` running `check_status.py --create-pr`.                                                                                                                                                                      |
