...
- **Status Discovery Recipe**: Run `./scripts/release/release_manager.py`.
...
| **`scripts/release/release_manager.py`**                    | **Implemented** | Centralized release status & PR creator script. Supports status inspection, git commit audit since last baseline tag, strict SemVer calculation, version string / changelog header mutations, lockfile sync, HTTP 404 registry handling, and PR creation via `gh`. Supports `--create-pr` and `--dry-run`. |
