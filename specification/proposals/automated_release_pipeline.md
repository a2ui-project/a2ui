...
- **Status Discovery Recipe**: Run `./.agents/skills/a2ui-release-sdks/scripts/check_status.py`.
...
### 2. Status Checker Script (`check_status.py`) Updates

Update `check_status.py` to:
...
## 7. Required Code & Workflow Changes

| Target File                                                    | Required Modification                                                                                           |
| :------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------- |
| **`specification/proposals/automated_release_pipeline.md`**    | Create this specification proposal document.                                                                    |
| **`.agents/skills/a2ui-release-sdks/scripts/check_status.py`** | Update tag parser to match `javascript/` and `python/` ecosystem tag formats.                                   |
| **`renderers/release.sh`**                                     | Support `javascript/<pkg>/v<version>` tag input parsing.                                                        |
| **`agent_sdks/python/release.sh`**                             | Support `python/<pkg>/v<version>` tag input parsing.                                                            |
| **`.agents/skills/a2ui-release-sdks/SKILL.md`**                | Update skill instructions with `javascript/` / `python/` tagging schemes and PR description commit audit rules. |
| **`docs/contributing/release.md`**                             | Update master release guide with tag naming conventions and CI/CD workflow architecture.                        |
| **`.github/workflows/release-pr.yml`**                         | Add workflow for weekly status check and release PR generation.                                                 |
| **`.github/workflows/tag-on-merge.yml`**                       | Add workflow to auto-create `javascript/` and `python/` Git tags when version PR merges.                        |
| **`.github/workflows/publish-tag.yml`**                        | Add workflow to publish packages on tag push with dry-run support.                                              |
