...
| **`.agents/skills/a2ui-release-sdks/scripts/check_status.py`**      | **Implemented** | Added tag resolution (`javascript/`, `python/`), direct npm HTTP API registry checks, line-by-line CHANGELOG section parser, git sync warning banner, and strict SemVer category evaluator.                                                                                |
| **`.agents/skills/a2ui-release-sdks/scripts/create_release_pr.py`** | **Implemented** | Created automated release PR generator script. Performs status inspection, git commit audit since last baseline tag, strict SemVer calculation, version string / changelog header mutations, `yarn install` lockfile sync, and PR creation via `gh`. Supports `--dry-run`. |
...
| **`.github/workflows/release-pr.yml`**                              | **Implemented** | Created GitHub Action workflow for weekly scheduled cron (`0 9 * * 1`) and `workflow_dispatch` running `create_release_pr.py`.                                                                                                                                             |
