...
2. It orders property keys deterministically using catalog-agnostic heuristics: required properties sorted alphabetically first, followed by optional properties sorted alphabetically, with `checks` appended at the end for checkable components. `checks` is placed at the end of the positional parameter list so that client-side validation rules can be naturally appended at the end of component statements without requiring placeholder underscores for preceding optional properties.
