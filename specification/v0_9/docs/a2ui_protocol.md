...
The whole loop runs on the client:
...
3. The client applies the results.
...
Transformations are built by nesting `FunctionCall` objects, not by evaluating an expression language. Nothing executable crosses the wire: the client only ever runs functions it already implements, and every argument is validated against the catalog schema before invocation.
