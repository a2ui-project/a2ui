...
- Defined explicit component resolution logic: the renderer checks the component-level `catalogId` first, then falls back to the surface default `catalogId`. If neither is defined, the renderer errors out and does not render the component. There is no fallback to catalogs declared in capabilities.
...
- Defined strict component and function catalog resolution logic:
  ...
  3. If neither exists, report an error and do not render the component (or fail the function call). There is no fallback to catalogs advertised in capabilities.
...
- Implement multi-catalog mixing by supporting components and function calls from any catalog in `supportedCatalogIds`.
