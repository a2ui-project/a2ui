...
- Supported target prefixes (`agent:` and `renderer:`) on `FunctionCall` names to explicitly direct function execution target routing without relying exclusively on implicit fallback resolution.
