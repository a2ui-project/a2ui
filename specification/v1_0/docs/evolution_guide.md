...
- Added a `version` field (e.g., `"version": "1.0"`) to catalog definition metadata (`catalog_definition.json`). If omitted, `version` defaults to `"0.9"` for backward compatibility; catalog definitions targeting `1.0` and beyond MUST specify `"version"`.
