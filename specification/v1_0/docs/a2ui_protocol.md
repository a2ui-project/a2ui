...
All catalog IDs referenced within a single `agentToRenderer` message (whether at the component, function-call, or surface level) must refer to catalogs that are compatible with that message's protocol version. Due to structural differences, v0.9 and v1.0 protocols and catalogs are mutually incompatible (a v1.0 message cannot resolve against a v0.9 catalog, nor can a v0.9 message resolve against a v1.0 catalog). Going forward, protocol evolution is anticipated to follow semantic versioning conventions, where future minor protocol versions (such as a future v1.1) will support v1.0 catalogs while leaving room to formalize the exact compatibility model in future releases.

Furthermore, a surface created with one protocol version may be updated over its lifetime by subsequent messages using a different protocol version (for example, a surface created with a v0.9 message may receive updates from a v1.0 message). In such cases, components in the updating message must resolve against a catalog compatible with that message's protocol version.
