...
function validateSchema(data, schemaName, matchers) {
    const errors = [];
    if (data.surfaceUpdate) {
        ...
        errors.push('A2UI Protocol message must have one of: surfaceUpdate, dataModelUpdate, beginRendering, deleteSurface.');
    }
    if (matchers) {
...
function validateDeleteSurface(data, errors) {
...
function validateSurfaceUpdate(data, errors) {
    if (data.surfaceId === undefined) {
...
function validateDataModelUpdate(data, errors) {
    if (data.surfaceId === undefined) {
    ...
    const validateValueProperty = (item, itemErrors, prefix) => {
        ...
        if (valueCount !== 1) {
            itemErrors.push(`${prefix} must have exactly one value property (${valueProps.join(', ')}), found ${valueCount}.`);
            return;
        }
        if (foundValueProp === 'valueMap') {
            ...
            item.valueMap.forEach((mapItem, index) => {
                if (!mapItem.key) {
                ...
                if (mapValueCount !== 1) {
                    itemErrors.push(`${prefix} 'valueMap' item at index ${index} must have exactly one value property (${mapValueProps.join(', ')}), found ${mapValueCount}.`);
                }
                const allowedMapKeys = ['key', ...mapValueProps];
                for (const key in mapItem) {
                    if (!allowedMapKeys.includes(key)) {
                        itemErrors.push(`${prefix} 'valueMap' item at index ${index} has unexpected property: ${key}`);
                    }
                }
            });
        }
    ...
    data.contents.forEach((item, index) => {
        ...
        for (const key in item) {
            if (!allowedKeys.includes(key)) {
                errors.push(`DataModelUpdate 'contents' item at index ${index} has unexpected property: ${key}`);
            }
        }
    });
}
function validateBeginRendering(data, errors) {
    if (data.surfaceId === undefined) {
...
function validateBoundValue(prop, propName, componentId, componentType, errors) {
    if (typeof prop !== 'object' || prop === null || Array.isArray(prop)) {
        errors.push(`Component '${componentId}' of type '${componentType}' property '${propName}' must be an object.`);
        return;
    }
    ...
    if (validKeyCount !== 1 || keys.length !== 1) {
        errors.push(`Component '${componentId}' of type '${componentType}' property '${propName}' must have exactly one key from [${allowedKeys.join(', ')}]. Found: ${keys.join(', ')}`);
    }
}
function validateComponent(component, allIds, errors) {
    if (!component.id) {
    ...
    if (componentTypes.length !== 1) {
        errors.push(`Component '${component.id}' must have exactly one property in 'component', but found ${componentTypes.length}.`);
        return;
    }
    ...
    switch (componentType) {
        ...
        case 'AudioPlayer':
            ...
            if (properties.description)
                validateBoundValue(properties.description, 'description', component.id, componentType, errors);
            break;
        case 'TextField':
            checkRequired(['label']);
            if (properties.label)
        ...
        case 'MultipleChoice':
            ...
            if (properties.selections) {
                if (typeof properties.selections !== 'object' ||
                    properties.selections === null ||
                    (!properties.selections.literalArray && !properties.selections.path)) {
                    errors.push(`Component '${component.id}' of type '${componentType}' property 'selections' must have either 'literalArray' or 'path'.`);
                }
            }
