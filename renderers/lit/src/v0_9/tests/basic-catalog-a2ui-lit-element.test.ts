...
describe('BasicCatalogA2uiLitElement', () => {
  ...
  it('should fallback to injecting a style tag when adoptedStyleSheets is unavailable', () => {
    ...
    try {
      ...
      document.body.appendChild(el1);
      const styleEls = document.head.querySelectorAll(`style[data-a2ui-tag="${tagName}"]`);
      assert.strictEqual(styleEls.length, 1, 'Should inject exactly one style element');
      assert.match(
        styleEls[0].textContent || '',
        new RegExp(`${tagName} \\{[^}]*border: 1px solid black`),
      );

      // Connect second element and verify deduplication
      document.body.appendChild(el2);
      const styleElsAfterSecond = document.head.querySelectorAll(
        `style[data-a2ui-tag="${tagName}"]`,
      );
      assert.strictEqual(
        styleElsAfterSecond.length,
        1,
        'Should not inject duplicate style elements',
      );
