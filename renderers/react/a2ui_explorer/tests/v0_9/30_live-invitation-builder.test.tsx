...
describe('Example: Live Invitation Builder', () => {
  ...
  beforeEach(async () => {
    ...
    container = await loadExample('30_live-invitation-builder.json', actionSpy);
    surface = getSurface(container);
    textContent = surface.textContent || '';
  });

  function getLivePreview() {
    const cards = Array.from(surface.querySelectorAll('.a2ui-card'));
  ...
  it('should render date time input', async () => {
    const dateTimeInput = surface.querySelector('.a2ui-datetime-input-webkit') as HTMLInputElement;
  ...
  it('should update preview when changing Event Name input', async () => {
    ...
    await act(async () => {
      changeInputValue(nameInput, 'Awesome Party');
      changeInputValue(guestInput, 'Alex Johnson');
      await whenSettled();
    });

    const updated = await waitForCondition(() => {
      return (getLivePreview().textContent || '').includes('Awesome Party');
    });
    expect(updated).withContext('Should update preview to Awesome Party').toBeTrue();
  });
  ...
  it('should update preview when changing Guest of Honor input', async () => {
    ...
    await act(async () => {
      changeInputValue(nameInput, 'Summer Gala');
      changeInputValue(guestInput, 'John Doe');
      await whenSettled();
    });

    const updated = await waitForCondition(() => {
      return (getLivePreview().textContent || '').includes('John Doe');
    });
    expect(updated).withContext('Should update preview to John Doe').toBeTrue();
  });

  it('should update preview when changing location', async () => {
    const chips = Array.from(surface.querySelectorAll('.chip')) as HTMLElement[];
    ...
    const ballroomUpdated = await waitForCondition(() => {
      const text = getLivePreview().textContent || '';
      return text.includes('Location: ballroom');
    });
    ...
    const terraceUpdated = await waitForCondition(() => {
      const text = getLivePreview().textContent || '';
      return text.includes('Location: terrace');
    });
