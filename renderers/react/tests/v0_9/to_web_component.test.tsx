...
import type {ReactComponentImplementation} from '../../src/v0_9/react_component_implementation';
...
describe('React toWebComponent', () => {
  ...
  it('uses custom tagName if defined on the component implementation', () => {
    const TagComp: ReactComponentImplementation = {
      name: 'ImplTagComp',
      schema: z.object({}),
      render: () => <div>Impl Tag</div>,
      tagName: 'custom-impl-react-tag',
    };
  ...
  it('renders React component and binds context correctly', async () => {
    ...
    const catalog = new Catalog('test-catalog', '0.9', [impl], []);
  ...
  it('renders child components with buildChild (both native and web component children)', async () => {
    ...
    const catalog = new Catalog('test-catalog', '0.9', [ChildWc, ParentWc, ChildNative], []);
