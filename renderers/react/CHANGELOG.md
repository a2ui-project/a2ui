...
- (v0_9) Unresolved child references are reported through the surface's `onError` (`UNRESOLVED_CHILD_REFERENCE`), once per reference, and the rendered notice names the cause: a missing schema marker, a missing component, or a data path the payload never created ([#2393](https://github.com/a2ui-project/a2ui/pull/2393)).
