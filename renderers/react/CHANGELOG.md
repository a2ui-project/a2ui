...
- **BREAKING CHANGE**: Importing from `@a2ui/react` (root `.`) now resolves to the modern evolving codebase (`v0.9` / `v1.0`) instead of `v0.8`. For legacy v0.8 imports, use `@a2ui/react/v0_8`. Backwards compatibility for `./v0_9` is maintained via re-export shims.
