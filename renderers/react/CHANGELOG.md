...
- **DEPRECATED**: (v0_9) A child reference whose schema property carries no component-id marker renders through a compatibility fallback and reports `UNMARKED_CHILD_REFERENCE` through `onError`, once per reference. Mark the property with `componentId()` or `childList()`; the fallback and the `DeferredChild` export will be removed in a future release. The `ChildList` union is recognized by shape, and a plain array of component ids by its elements' markers ([#2393](https://github.com/a2ui-project/a2ui/pull/2393)).
