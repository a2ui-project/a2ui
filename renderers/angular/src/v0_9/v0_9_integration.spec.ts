...
/**
 * Normalizes imported JSON mock payloads into a flat array of {@link A2uiMessage}.
 *
 * Depending on TypeScript compiler and module resolution settings (such as synthetic
 * default imports vs direct JSON module imports in test runners), imported JSON files
 * may be resolved either as a direct array, wrapped in an object under a `default`
 * property, or represented as a dictionary of message entries. This helper normalizes
 * those variations to guarantee a uniform sequence of messages for integration tests.
 *
 * @param mock The imported raw JSON mock payload.
 * @returns An array of parsed A2UI messages.
 */
