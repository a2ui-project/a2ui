...
import {AngularCatalog, createComponentImplementation, getUniversalWebComponent} from './types';
...
describe('AngularCatalog & Catalog Types', () => {
  ...
  it('records the pre-built universal Web Component internally when componentApi is a WebComponentImplementation', () => {
    class MockCustomWcElement extends (typeof HTMLElement !== 'undefined' ? HTMLElement : Object) {}
    const wcApi = {
      name: 'CustomWcItem',
      schema: z.object({}),
      tagName: 'a2ui-custom-wc-item',
      element: MockCustomWcElement as CustomElementConstructor,
    };
    ...
    expect(getUniversalWebComponent(impl)?.tagName).toBe('a2ui-custom-wc-item');
    expect(getUniversalWebComponent(impl)?.element).toBe(
      MockCustomWcElement as CustomElementConstructor,
    );
  });

  it('returns no universal Web Component when componentApi is a plain ComponentApi', () => {
    @Component({
    ...
    expect(getUniversalWebComponent(impl)).toBeUndefined();
  });
});
