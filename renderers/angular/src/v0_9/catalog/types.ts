...
// Internal WeakMap to associate pre-built universal Web Component implementations with Angular components
...
const componentToUniversalWcMap = new WeakMap<
  Type<CatalogComponentInstance>,
  WebComponentImplementation
>();

/**
 * Internal helper to retrieve the pre-built universal Web Component implementation associated with an
 * AngularComponentImplementation, if one was provided during createComponentImplementation.
 *
 * @internal
 */
export function getUniversalWebComponent(
  impl: AngularComponentImplementation,
): WebComponentImplementation | undefined {
  return componentToUniversalWcMap.get(impl.component);
}
...
export function createComponentImplementation(
  ...
  if (isWebComponentImplementation(componentApi)) {
    componentToUniversalWcMap.set(component, componentApi);
  }
