...
- (v0_9) `GenericBinder` schema scraping now classifies a `Dynamic*` schema composed into a wider union as `DYNAMIC`, so bindings on such properties (e.g. the basic catalog's `DateTimeInput` `min`/`max`) resolve and subscribe instead of passing through as raw `{path}` objects. [#2531](https://github.com/a2ui-project/a2ui/pull/2531)
