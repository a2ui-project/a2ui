...
- (v0_9) `getClientCapabilities` now advertises catalog aliases alongside canonical IDs in `supportedCatalogIds`, each catalog's canonical ID first, so agents matching on a legacy ID still recognize the client. `inlineCatalogs` remains keyed by canonical ID only.
